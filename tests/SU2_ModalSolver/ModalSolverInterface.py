...
class modalInterpreter(SolidSolver):
    ...
    def __init__(self, confFile, initialModalDisp, initialModalVel, modalMass, modalDamping, modalStiffness):
        ...
        self.y0 = np.concatenate((initialModalDisp, initialModalVel)) # initial conditions for the modal coordinates and the modal velocities
        self.Mq = modalMass
        ...
        self.Phi = np.concatenate((self.nodalMod_X, self.nodalMod_Y, self.nodalMod_Z)) # modal matrix    
        self.PhiT = self.Phi.transpose()
        
        self.x = np.dot(self.Phi, self.y0[0:self.nModes]) # initial displacement vector
        ...
        self.f = np.concatenate((self.nodalLoad_X, self.nodalLoad_Y, self.nodalLoad_Z)) # initial force vector
        self.fq = np.dot(self.PhiT, self.f) # initial modal force vector
