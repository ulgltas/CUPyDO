...
{
    if (dim > nDim)
        throw out_of_range("Exceed node dimension");
    minCoords[dim] = val;
}
...
void ADT_BaseType::buildADT(int nDim, int nPoints, double *coord)
{
    ...
    nLeaves = nPoints - 1;
    if (nPoints <= 1)
        ++nLeaves; //If 0 then nLeaves = 0, if 1 then nLeaves = 1
    if (nLeaves == 0)
    {
        empty = true;
        return;
    ...
    while (1)
    {
        ...
        for (int i = 0; i <= nLeavesToDivide; ++i)
            nPointIDs[i] = nPointIDsNew[i];
        for (int i = 0; i < nLeavesToDivide; ++i)
            curLeaf[i] = curLeafNew[i];
        for (int i = 0; i < nPointIDs[nLeavesToDivide]; ++i)
            pointIDs[i] = pointIDsNew[i];
    }
}
...
double ADT_BaseType::computeDistanceSquare(double *pointA, ADT_Node &BBox)
{
    ...
    {
        ds = 0.0;
        if (pointA[dim] < BBox.getValMinCoords(dim))
            ds = pointA[dim] - BBox.getValMinCoords(dim);
        else if (pointA[dim] > BBox.getValMaxCoords(dim))
            ds = pointA[dim] - BBox.getValMaxCoords(dim);
...
bool ADT_BaseType::intersectSphere(double *center, const double &radius, ADT_Node &BBox)
{
    ...
    if (distanceSquare == 0)
    { //Sphere center is included in the box, so we have intersection
        return true;
    }
    else
    {
        if (pow(radius, 2) < distanceSquare)
            return false; // The sphere is completely outside of the box
        else
            return true;
    }
}
