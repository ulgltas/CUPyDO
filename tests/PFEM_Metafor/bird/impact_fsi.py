...
def main(_p, nogui):
    ...
    algorithm = cupyalgo.AlgorithmBGSAitkenRelax(manager, fluidSolver, solidSolver, interpolator,
                                                 criterion, p['nFSIIterMax'], p['dt'], p['tTot'], p['timeIterTreshold'], p['omegaMax'], comm)
    ...
    tests.add(CTest('Mean nb of FSI iterations',
                    algorithm.getMeanNbOfFSIIt(), 2, 1, True))  # abs. tol. of 1
    tests.add(CTest('Y-displacement panel center',
                    result_1[2], -0.001462, 1e-2, False))  # rel. tol. of 1%
    tests.run()
