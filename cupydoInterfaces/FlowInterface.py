...
class FlowSolver(FluidSolver):
	...
	def __setCurrentState(self):
		...
		# elemental forces: integrate pressure from nodes
		fxE = {}
        fyE = {}
        fzE = {}
		for e in self.boundary.elems:
			cp = 0
			for n in e.nodes:
				cp += self.flow.Solver.Cp[n.row]
			cp /= e.nodes.size()
			sur = e.computeV()
			fxE[e] = p * cp * e.normal().x[0] * sur #TODO
			fyE[e] = p * cp * e.normal().x[1] * sur
			fzE[e] = p * cp * e.normal().x[2] * sur
