...
def getFsiP():
    ...
    p['criterion'] = 'norm'                 # convergence criterion available: Norm, Relative 
    p['algorithm'] = 'IQN_ILS'              # FSI algorithms available: Explicit, StaticBGS, AitkenBGS, IQN_ILS 
    # FSI parameters
    p['regime'] = 'steady'                  # steady or unsteady 
    p['nDim'] = 3                           # dimension: 2 or 3 
    p['dt'] = 0.0                           # time steps (if unsteady) 
    p['tTot'] = 0.0                         # total time
    p['timeItTresh'] = -1                   # ?
    p['mechanicalTol'] = 1e-5               # tolerance on displacements (required by BGS)
    ...
    p['interpType'] = 'conservative'        # Conservative interpolation
    p['mechanical'] = True                  # Enable mechanical coupling
    p['thermal'] = False                    # Disable thermal coupling
    p['dtSave'] = 0                         # Time steps for saving results
    p['qrFilter'] = 'Haelterman'            # Filtering tcehnique for the FSI algorithm
